...
Airplane::Airplane(string type) : type(type)
{
    ...
    Airplane::emergencySequenceInitiated = false;
}
...
bool Airplane::isValid()
{
    ...
    if (Airplane::size.empty() || Airplane::fuel == 0 || Airplane::model.empty() || Airplane::number.empty() || Airplane::callsign.empty() || (Airplane::state != incoming && Airplane::state != gTechnicalCheck) || Airplane::engine.empty())
    {
        return false;
    }
...
bool Airplane::isEmergencySequenceInitiated() const
{
    return Airplane::emergencySequenceInitiated;
}
void Airplane::setEmergencySequenceInitiated(bool emergencySequenceInitiated)
{
    Airplane::emergencySequenceInitiated = emergencySequenceInitiated;
    ENSURE(Airplane::emergencySequenceInitiated == emergencySequenceInitiated, "emergency initiated set");
}

// functies
void Airplane::pushBack()
{
    REQUIRE(Airplane::getCurrentTask() == "IFR" || Airplane::getCurrentTask() == "pushback" || Airplane::getCurrentTask() == "request taxi", "correct state");
    ...
    if (!Airplane::taxiPoint.empty())
    {
        if (Airplane::taxiPoint == Airplane::taxiRoute->getTaxiPoints()[crossingIndex])
        {
            ...
            else if (Airplane::state == taxiPointCMS)
            {
                ...
                Airplane::getAirport()->getRunway(taxiRoute->getTaxiCrossings()[crossingIndex])->setHoldingShortOccupied(false);
        ...
        if (Airplane::taxiCrossing == Airplane::taxiRoute->getTaxiCrossings()[crossingIndex])
        {
            ...
            else if (Airplane::state == taxiCrossingCMS)
            {
                if (Airplane::getAirport()->getRunway(Airplane::taxiCrossing)->getPermissionToCross())
                {
                    ...
                    Airplane::getAirport()->getRunway(Airplane::taxiCrossing)->setHoldingShortOccupied(false);
            ...
            else if (Airplane::state == taxiCrossingNF)
            {
                ...
                if (crossingIndex + 1 == (signed)taxiRoute->getTaxiCrossings().size())
                {
                    Airplane::airport->getRunways()[Airplane::airport->getRunways().size() - 1]->setHoldingShortOccupied(true);
                    Airplane::airport->getRunways()[Airplane::airport->getRunways().size() - 1]->setCrossing(false);
                }
                else
                {
                    Airplane::airport->getRunway(taxiRoute->getTaxiCrossings()[crossingIndex + 1])->setHoldingShortOccupied(true);
                    Airplane::airport->getRunway(taxiRoute->getTaxiCrossings()[crossingIndex + 1])->setCrossing(false);
                }
...
void Airplane::taxiToGate(int gate)
{
    ...
    if (!Airplane::taxiPoint.empty())
    {
        if (Airplane::taxiPoint == Airplane::taxiRoute->getTaxiPoints()[crossingIndex])
        {
            if (crossingIndex == 0)
            {
                ...
                {
                    Airplane::airport->getRunways()[0]->setHoldingShortOccupied(false);
                    Airplane::runway = NULL;
                    Airplane::operationTime = 1;
    ...
    else if (!Airplane::taxiCrossing.empty())
    {
        if (Airplane::taxiCrossing == Airplane::taxiRoute->getTaxiCrossings()[crossingIndex])
        {
            ...
            {
                Airplane::getAirport()->getRunway(Airplane::taxiCrossing)->setPermissionToCross(true);
                Airplane::getAirport()->getRunway(Airplane::taxiCrossing)->setCrossing(false);
...
void Airplane::emergencyLanding(Airport *airport)
{
    ...
    if (Airplane::currentTask != "emergency crash" && currentTask != "emergency landing")
    {
        ...
        if (Airplane::state == outOfFuel)
        {
            if (Airplane::attemptRunway != NULL)
            {
                Airplane::attemptRunway->setOccupied(false);
                Airplane::attemptRunway->setPermissionToCross(true);
            }
            Airplane::emergencySequenceInitiated = true;
        ...
        else
        {
            ...
            {
                if (airport->getFreeRunways().size() == 0)
                {
                    Airplane::state = outOfFuel;
                    return;
                }
                else
                {
                    Airplane::attemptRunway = airport->getRunways()[airport->getFreeRunways()[0]];
                    EmergencyAbove3000ftMessage(this, Airplane::attemptRunway, tijd);
                    Airplane::attemptRunway->setPermissionToCross(false);
                    Airplane::attemptRunway->setOccupied(true);
    ...
    else
    {
        ...
        {
            Airplane::runway = Airplane::attemptRunway;
            Airplane::airport = airport;
            Airplane::attemptRunway = NULL;
            Airplane::runway->setPermissionToCross(true);
...
{
    REQUIRE(Airplane::getCurrentTask() == "try to land" || Airplane::getCurrentTask() == "landing" || Airplane::getCurrentTask() == "descending to 5000ft." || Airplane::getCurrentTask() == "descending to 3000ft." || Airplane::getCurrentTask() == "descending to 0ft.", "correct task");
    REQUIRE(Airplane::getHeight() == 0 || Airplane::getHeight() == kHeightLevelA || Airplane::getHeight() == kHeightLevelB || Airplane::getHeight() == kHeightLevelC || Airplane::getHeight() == kHeightLevelD, "correct height");
                ...
                else if (Airplane::state == descendBRMS)
                {
                    ...
                    else if (Airplane::getEngine() == "propeller")
                    {
                        Airplane::operationTime = (Airplane::kHeightLevelB - Airplane::kHeightLevelC) /
                                                      Airplane::kProprellerDescentionSpeed -
                                                  1;
                    }
                }
                Airplane::state = descending;
...
{
    REQUIRE(Airplane::getCurrentTask() == "taking off" || Airplane::getCurrentTask() == "lift off" || Airplane::getCurrentTask() == "at holding point", "correct state");
    ...
    if (Airplane::currentTask == "lift off")
    {
        Airplane::currentTask = "taking off";
    }
    else if (Airplane::currentTask == "at holding point")
    {
        ...
        {
            if (!Airplane::runway->isOccupied())
            {
                if (!Airplane::runway->getWaitingOnRunway())
                {
                    if (!Airplane::runway->getCrossing())
                    {
                ...
                {
                    //plane already lined up
                    if (Airplane::waitOnRunway && !Airplane::runway->getCrossing())
                    {
                        clearedForTakeOffMessage(this, Airplane::runway, tijd);
                        Airplane::permissionToTakeOff = true;
                        Airplane::state = holdingPointMMS;
...
void Airplane::exitPlane()
{
    ...
    REQUIRE(Airplane::atGate() || Airplane::isEmergencySequenceInitiated(), "at gate or emergency");
    ...
    if (Airplane::passengers <= 0)
    {
        ...
        {
            logMessage(intToString(Airplane::passengerCapacity) + " passengers exited " + Airplane::callsign + " at gate " + intToString(Airplane::gate) + " of " + Airplane::airport->getName());
        }
        else
        {
            logMessage(intToString(Airplane::passengerCapacity) + " passengers exited " + Airplane::callsign + " at gate / of " + Airplane::airport->getName());
        }
...
void Airplane::enterPlane()
{
    ...
    if (Airplane::passengers >= Airplane::passengerCapacity)
    {
        ...
        logMessage(intToString(Airplane::passengerCapacity) + " passengers entered " + Airplane::callsign + " at gate " + intToString(Airplane::gate) + " of " + Airplane::airport->getName());
...
void Airplane::technicalCheck()
{
    ...
    if (Airplane::state == eTechnicalCheck)
    {
        ...
        Airplane::setOperationTime(ceil((Airplane::fuelCapacity - Airplane::fuel) / Airplane::kFuelPerMinute) + 2);
...
void Airplane::refuel()
{
    ...
    REQUIRE(atGate() || Airplane::emergencySequenceInitiated, "at gate or emergency");
    REQUIRE(Airplane::flightPlan != NULL, "flightplan assigned");
...
void Airplane::execTask(Airport *airport)
{
    ...
    else if (Airplane::currentTask == "emergency crash" || Airplane::currentTask == "emergency landing" || Airplane::currentTask == "crash")
    {
        ...
        Airplane::emergencyLanding(airport);
    }
    ...
    else if (Airplane::currentTask == "lift off")
    {
        Airplane::takeOff();
    }
}
...
void Airplane::continueTask(Airport *airport)
{
    ...
    if (Airplane::currentTask == "emergency crash" || Airplane::currentTask == "emergency landing" || Airplane::currentTask == "crash")
    {
        Airplane::descend(airport);
    }
}
